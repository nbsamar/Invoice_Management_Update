class InvoicesController < ApplicationController
  ...
  def pending
    ...
    # binding.pry
    invoices_with_due = ActiveRecord::Base.connection.execute("select * from (select inc.id, inc.amount, inc.reference, co.invoice_id as co_id, co.amount as co_amnt, co.reference as co_ref,(inc.amount - co.amount) / 2 as due from collections co, invoices inc where co.invoice_id = inc.id) z where due > 0;")
